...
myfinaltable=function(my.tables){
...
mytempfactor.ui.server=function(my.tables,tempnum,myvar,myLabels,myDF,myDFname,cutoff,should.comma){
  ...
  my.temp.server.file=capture.output(cat("observe({
                                         ...
                                         myfinaltable(my.tables[[",tempnum,"]])
...
make.me.shiny=function(myvar,myDF.name,myLabels,myDF2,mycutoff,my.tables){
  tempnum=1
  while(tempnum<{length(myvar)})
  {
    if(tempnum==1){
      my.temp.files1=mytempfactor.ui.server(my.tables,tempnum,myvar,myLabels,myDF2,myDF.name,cutoff=mycutoff,should.comma=T)
  ...
  my.temp.files1=mytempfactor.ui.server(my.tables,tempnum,myvar,myLabels,myDF2,myDF.name,cutoff=mycutoff,should.comma=F)
  my.temp.ui[tempnum]=my.temp.files1[1]
  my.temp.server[tempnum]=my.temp.files1[2]
...
label.explor.r=function(myDF,caseid,mycutoff=5,objections=NULL){
  ...
  #### Check for caseid in there
  my.int.list=1:dim(myDF2)[2]
  flag.int=0
  if(length(objections)>0){
    for(i in 1:length(objections)){
      objection.grep=grep(objections[i],names(myDF2))
      if(objection.grep==caseidnumber){
        flag.int=1
      }
      objection.grep.int=grep(objection.grep,my.int.list)
      my.int.list=my.int.list[-objection.grep.int]
    }
  }
  # if(flag.int==0){
  #   objection.grep.int=grep(caseidnumber,my.int.list)
  #   my.int.list=my.int.list[-objection.grep.int]
  # }
  #print(my.int.list)
  for(i in my.int.list){
    myDF2[[i]]=as.numeric(myDF2[[i]])
    int.grep=grep(paste("^",names(myDF2)[i],"$",sep=""),names(myDF))
    attributes(myDF2[[i]])=attributes(myDF[[int.grep]])
  }
  ...
  #### Make all tables ####
  my.tables=NULL
  for(i in 1:dim(myDF2)[2]){
    tables.grep=grep(paste("^",myVars[[i]],"$",sep=""),names(myDF2))
    #print(tables.grep)
    my.tables[[i]]=table(myDF2[[tables.grep]])

  }
  #print(my.tables[[15]])
  #mytable=my.tables
  #### Make the rest of the sources ####
  myTemps=make.me.shiny(myVars,"myDF2",myLabels,myDF2,mycutoff,my.tables)
  ...
  label.explor.r.my.selected=runApp(list(
    ui=dashboardPage(dashboardHeader(title="label.explor.R"),
                     dashboardSidebar(
                       ...
                       source(mybigcheckbox[1],local=T)[1]
                     ...
                     dashboardBody(useShinyjs(),
                                   ...
                                   div(style="display:inline-box",span(align="right",actionButton("commit","Update"),actionButton("selectall","Select All"),actionButton("reset","Reset")),
                                       span(align="right",
                                            actionButton("save",tagList("Save",icon("shiny-o")))
                                       )),
                                   p(),
                                   hr(),
  ...
  #### SINK ####
  #sink(file=file.choose(),type="output",split=T)
...
#' Recode SPSS Imports to NA
#'
#' Tibbles and car::recode do not work. This is for
#' taking care of negative values. Works on whole
#' data frame.
#'
#' @param myDF Dataframe
#' @param myNum numbers to be made NA using c()
#' @return Recoded Data Frame
#' @keywords Explore
#' @export
#' @examples
#' spss.na.recode
spss.na.recode=function(myDF,myNum){
  mynum=as.list(myNum)
  for(i in 1:length(colnames(myDF))){
    TFList=lapply(myDF[[i]], function(r) any(r %in% mynum))
    last.temp=1
    while(last.temp<{length(TFList)+1}){
      if(TFList[[last.temp]]){
        myDF[last.temp,i]=NA
